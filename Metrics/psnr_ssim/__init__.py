...
def normalize(img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    if type(img) == np.ndarray:  # (h,w,c) numpy (0,255)
        ...
        img -= mean
        img /= std
    if type(img) == torch.Tensor:  # (c,h,w) tensor (0,1)
...
def _calculate_psnr_ssim_niqe(output,target,cal_psnr=True,cal_ssim=True,cal_niqe=False, rgb_range=1,crop_border=0, input_order='CHW', test_y_channel=True,mean=(0,0,0), std=(1,1,1)):
    ...
    output_np = single2uint8(denormalize(output.data.cpu().numpy().astype(np.float32),mean=mean,std=std))
    target_np = single2uint8(denormalize(target.data.cpu().numpy().astype(np.float32),mean=mean,std=std))
    # else:
