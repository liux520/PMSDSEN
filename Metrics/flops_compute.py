...
__all__ = ['compute_time', 'compute_flops', 'compute_parameters']


def dict_json(opt, para, flops, filename='arguments.json'):
    path = opt['record_train']
    with open(os.path.join(path, filename), 'a+') as outfile:
        arg_dict = opt
        arg_dict['model_params'] = '{} '.format(para)
        arg_dict['flops'] = '{} '.format(flops)
        arg_dict['time'] = '{} '.format(compute_time())
...
def compute_parameters(model):
    return sum([para.numel() for para in model.parameters()]) / 1e6
...
def compute_flops(model, input=None):
    input = input if input is not None else torch.Tensor(1, 3, 64, 64)
