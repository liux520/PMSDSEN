...
if CSUPPORT:
    try:
        from Data.cityscapesscripts.evaluation import addToConfusionMatrix
    except:
        CSUPPORT = False
print(CSUPPORT)
...
### ---------------- Notice ---------------- ###
### You only need change !!!{predict_path}!!! path in line 77!

ROOTPATH = os.path.abspath(os.path.join(__file__, os.path.pardir, os.path.pardir))

# 1. Prediction Results:#
predict_path = r'D:\Software\Professional\AShare\Project\AServer\Seg\ASEG\results\cityscapes\val_418_7310\submit'
#r'D:\Software\Professional\AWork\Paper\MyWorks\Light_Seg\Compare_model\DABNet-master\result\My_submit'
#os.path.join(ROOTPATH, 'results','cityscapes', 'val', 'labelid')

# 2. Cityscapes Dataset Path:
city_path = os.path.join(ROOTPATH, 'Datasets', 'Cityscapes')
...
def getPrediction( args, groundTruthFile ):
    ...
    if not args.predictionPath:
        ...
        if 'CITYSCAPES_RESULTS' in os.environ:
            ...
            rootPath = predict_path #os.path.join(ROOTPATH, 'results','cityscapes', 'val_id')
            print(f'pred:{rootPath}') #os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..','..','results','cityscapes', 'val_id')
...
if 'CITYSCAPES_DATASET' in os.environ:
    ...
    args.cityscapesPath = city_path #os.path.join(ROOTPATH, 'Datasets', 'Cityscapes')
        #os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..', '..', 'Datasets', 'Cityscapes', 'gtFine')
...
print(f'gt:{args.groundTruthSearch}')
