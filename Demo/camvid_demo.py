...
def CamvidDataloader(train_batchsize=1,val_batchsize=1,crop_size=(480,360),sr_scale=1,rescale=(0.75,2),**kwargs):
    train_set = CamvidSegmentation(crop_size=crop_size,sr_scale=sr_scale,rescale=rescale,mode='trainval')
    val_set = CamvidSegmentation(crop_size=crop_size,sr_scale=sr_scale,rescale=rescale,mode='val')
...
class CamvidSegmentation(Dataset):
    ...
    def __init__(self,crop_size, sr_scale=2, rescale=(0.75,2), ignore_label=255 ,mode='train'):
        ...
        dataset_path = os.path.join(rootpath, f'Datasets{os.sep}CamVid{os.sep}CamVid_PIDNet_archive')

        # Image:
        if self.mode == 'train':
            self.images = [os.path.join(dataset_path, 'train', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'train')))]
            self.labels = [os.path.join(dataset_path, 'trainannot', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'trainannot')))]
        elif self.mode == 'val':
            self.images = [os.path.join(dataset_path, 'val', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'val')))]
            self.labels = [os.path.join(dataset_path, 'valannot', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'valannot')))]
        elif self.mode == 'test':
            self.images = [os.path.join(dataset_path, 'test', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'test')))]
            self.labels = [os.path.join(dataset_path, 'testannot', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'testannot')))]
        elif self.mode == 'trainval':
            self.images = [os.path.join(dataset_path, 'train', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'train')))] +\
                          [os.path.join(dataset_path, 'val', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'val')))]
            self.labels = [os.path.join(dataset_path, 'trainannot', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'trainannot')))] + \
                          [os.path.join(dataset_path, 'valannot', i) for i in natsorted(os.listdir(os.path.join(dataset_path, r'valannot')))]
            self.images, self.labels = natsorted(self.images), natsorted(self.labels)

    def __getitem__(self, item):
        impth = self.images[item]
        lbpth = self.labels[item]
        if os.path.basename(impth) != os.path.basename(lbpth):
            assert os.path.basename(impth) != os.path.basename(lbpth), f'{os.path.basename(impth)}, {os.path.basename(lbpth)} '
